...
const ReadyButton = ({players, onClickReady, onClickCancel, status, isHost }) => {
    const readyPlayers = players.filter(player => player.status == "ready")
    
    return(
        <Container className="readyButtonWrapper">
            {status === "ready" ?
                <Button className="readyButton" onClick={onClickCancel}>취소하기</Button> :
                <Button className="readyButton" onClick={onClickReady}>{isHost? "시작하기": "준비하기"}</Button>
            }
